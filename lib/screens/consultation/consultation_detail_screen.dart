...
class _ConsultationDetailScreenState extends State<ConsultationDetailScreen> {
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    _localConsultation.status.replaceAll('_', ' '),
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
            ...
            Wrap(
              ...
              children: [
                _buildInfoChip(
                  Icons.category,
                  _localConsultation.category.replaceAll('_', ' '),
                ),
                _buildInfoChip(
                  Icons.priority_high,
                  'Priority: ${_localConsultation.priority}',
                ),
                if (_localConsultation.isPremium)
                  _buildInfoChip(
                    Icons.star,
                    'Premium',
                    color: Colors.amber,
                ...
                _buildInfoChip(
                  ...
                  _formatDate(_localConsultation.createdAt),
                ),
              ],
            ),
  ...
  Widget _buildInfoChip(IconData icon, String text, {Color? color}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: color?.withValues(alpha: 0.3) ?? AppColors.textSecondary.withValues(alpha: 0.3),
        ),
      ),
      child: Row(
  ...
  Widget _buildActionButtons() {
    ...
    return Row(
      children: [
        if (isAgronomist && _localConsultation.status == 'PENDING' &&
        ...
        if ((_localConsultation.expert?.id == authProvider.user?.id || 
             _localConsultation.farmer?.id == authProvider.user?.id) &&
            (_localConsultation.status == 'ACCEPTED' ||
        ...
        if (_localConsultation.farmer?.id == authProvider.user?.id && 
            _localConsultation.status == 'PENDING' &&
